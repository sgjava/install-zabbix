...
if [ "$db_populated" != "1" ]; then
	...
	# Enable function creation and disable strict mode for the entire schema/images/data import
	$MYSQL_CMD -e "SET GLOBAL log_bin_trust_function_creators = 1; SET GLOBAL innodb_strict_mode = OFF;" >> "$logfile" 2>&1

	$MYSQL_CMD zabbix < schema.sql >> "$logfile" 2>&1
	$MYSQL_CMD zabbix < images.sql >> "$logfile" 2>&1
	$MYSQL_CMD zabbix < data.sql >> "$logfile" 2>&1

	# Reset MySQL session flags after all 3 files are loaded
